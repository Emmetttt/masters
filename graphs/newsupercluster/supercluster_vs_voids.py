...
while k < len(SNered): ##Find the nearest sc for each k Supernova
    while i < len(clusterz):
        ...
        i+=1
    while j < len(voidRA):
        distancesNew = fun.polar_dist_comp(SNeRA[k], voidRA[j], SNedec[k], voiddec[j], SNered[k], voidz[j])
        if fun.D(fun.HubbleIntegrate(distancesNew)) < voidRadii[j]:
            scout.append([SNedist[k], SNedisterror[k], SNered[k], voidRadii[j], fun.D(fun.HubbleIntegrate(distancesNew)) - voidRadii[j]])
            j = len(voidRA)+1
        j+=1
    j=0
    i=0
