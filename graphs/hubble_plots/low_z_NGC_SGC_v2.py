...
NGCH = [fun.taylorHubble(x,70.25) for x in redshiftrange]
SGCH = [fun.taylorHubble(x,72.35) for x in redshiftrange]

##Graph for redshift vs distance
plt.figure()
plt.errorbar(NGCred, NGCdist, yerr=NGCdisterror, fmt='.', markersize=5, color="blue", label='NGC')
plt.errorbar(SGCred, SGCdist, yerr=SGCdisterror, fmt='.', markersize=5, color="red", label='SGC')
plt.plot(redshiftrange, SGCH, '-', color="red", label='H0 = 70.3 ± 1.6')
plt.plot(redshiftrange, NGCH, '-', color="blue", label='H0 = 72.4 ± 2.5')
...
plt.figure(2)
plt.errorbar(NGCred, NGC_H, yerr=NGC_dH, fmt='.', markersize=5, color="blue", label='NGC')
plt.errorbar(SGCred, SGC_H, yerr=SGC_dH, fmt='.', markersize=5, color="red", label='SGC')
plt.plot(redshiftrange, 10*[69.9], '-', color="blue", label='H0 = 72.4 ± 2.5')
plt.plot(redshiftrange, 10*[69.9+2.5], ':', color="blue", alpha=0.3)
plt.plot(redshiftrange, 10*[69.9-2.5], ':', color="blue", alpha=0.3)
plt.plot(redshiftrange, 10*[66.9], '-', color="red", label='H0 = 70.3 ± 1.6')
...
plt.title('Hubble values for NGC and SGC')
plt.legend()
plt.show()
...
emptyNGC = plt.hist([], range=[-0.25,0.25], alpha=0.5, label='$N=%i, \mu=%.2f \pm %.2f$, $\sigma=%.2f$'%(len(NGC_dH), NGCmu, NGCSE, NGCsigma), color='white')
...
SGCSE = SGCsigma/np.sqrt(len(SGC_dH))
emptySGC = plt.hist([], range=[-0.25,0.25], alpha=0.5, label='$N=%i, \mu=%.2f \pm %.2f$, $\sigma=%.2f$'%(len(SGC_dH), SGCmu, SGCSE, SGCsigma), color='white')
plt.xlabel('$\delta d_L$')
plt.ylabel('Frequency')
plt.title('Histogram of deviation from expected distance for NGC and SGC supernovae')
plt.legend()
plt.show()
...
plt.title('Cumulative frequency for the NGC and SGC deviations.')
plt.legend()
plt.show()
